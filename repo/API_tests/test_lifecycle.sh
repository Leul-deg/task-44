...
RESP=$(curl -s -o /tmp/job.json -w "%{http_code}" -X POST "$BASE/api/jobs" -b /tmp/emp.txt \
  ...
  -d '{"title":"Full Stack Developer","description":"Join our team to build modern web applications using Vue.js and Spring Boot framework.","categoryId":9,"locationId":1,"payType":"HOURLY","settlementType":"WEEKLY","payAmount":50.00,"headcount":2,"weeklyHours":35,"contactPhone":"5559876543","tags":["vue","spring"]}')
...
RESP=$(curl -s -o /tmp/job2.json -w "%{http_code}" -X POST "$BASE/api/jobs" -b /tmp/emp.txt \
  -H "Content-Type: application/json" -H "X-XSRF-TOKEN: $EMP_CSRF" \
  -d '{"title":"QA Engineer","description":"Test our platform for quality assurance, write automated tests, and ensure reliability.","categoryId":9,"locationId":1,"payType":"HOURLY","settlementType":"WEEKLY","payAmount":40.00,"headcount":1,"weeklyHours":30,"contactPhone":"5551112222","tags":["testing"]}')
JOB2=$(jpf /tmp/job2.json id)
S=$(jpf /tmp/job2.json status)
